...
import cs4620.ray1.BlockSpiral;
	...
	public static void renderBlock(Scene scene, Image outImage, int offsetX, int offsetY, int sizeX, int sizeY) {
		...
		for(int x = offsetX; x < (offsetX + sizeX); x++) {
			for(int y = offsetY; y < (offsetY + sizeY); y++) {
				...
				//Loop through samples on x and y, get sub-rays
				for (int x_i = 0; x_i < samples; x_i++){
					double x_ray = startX + sInvD2 + sInv*x_i;
					for (int y_i = 0; y_i < samples; y_i++){
						double y_ray = startY + sInvD2 + sInv*y_i;
						
						cam.getRay(ray,  x_ray / width, y_ray / height);
						shadeRay(rayColor, scene, ray, 1);
						
						//Only take fraction of the color per ray
						rayColor.set(rayColor.mul(sInvSqr));
