...
public class BvhNode {
	...
	public boolean intersects(Ray ray) {
		...
		Vector3d tmin = (minBound.clone().sub(ray.origin)).div(ray.direction);
		Vector3d tmax = (maxBound.clone().sub(ray.origin)).div(ray.direction);
		
		double tEnterX = Math.min(tmin.x, tmax.x);
		double tExitX = Math.max(tmin.x, tmax.x);
		
		double tEnterY = Math.min(tmin.y, tmax.y);
		double tExitY = Math.max(tmin.y, tmax.y);
		
		double tEnterZ = Math.min(tmin.z, tmax.z);
		double tExitZ = Math.max(tmin.z, tmax.z);
		
		double tEnter = Math.max(tEnterX, Math.max(tEnterY, tEnterZ));
		double tExit = Math.min(tExitX, Math.min(tExitY, tExitZ));
		
		//It intersects if there's area between enter and exit
		return tEnter <= tExit && tEnter < ray.end && tExit > ray.start;
	}
}
