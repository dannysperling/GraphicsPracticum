...
public class RayTracer {
	...
	public static void shadeRay(Colord outColor, Scene scene, Ray ray) {
		...
		boolean intersectionOccured = scene.getFirstIntersection(intersectionRecord, ray);
		
		//If nothing hit, use back color
		if (!intersectionOccured){
			outColor.set(scene.backColor);
			return;
		}
		
		//Otherwise, get the color from the shader's shade method
		intersectionRecord.surface.getShader().shade(outColor, scene, ray, intersectionRecord);
	}
}
